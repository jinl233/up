...
- 对同步信号量empty和full 的wait和signal操作，同样需要成对出现，但它们分别处于不同的进程中。例如， wait(empty)在计算进程中，而signal(empty)则在打印进程中，计算进程若因执行wait(empty)而阻塞，则以后将由打印进程唤醒它  
- 每个程序中的多个wait操作的顺序不能颠倒，应先执行对同步信号措的wait操作，再执行对互斥信号屉的wait操作，否则可能会引起进程死锁。
...
不允许write嗖t程去写。因此，仅当readcount=O表示尚无reade嗖t程在读时， reader进程才需要执
行wait(wmutex)操作。若wait(wmutex)操作成功， reader进程便可去读，相应地做readcount+ 1 操
作。同理， reade设t程仅当在执行了readcount— 1操作后其值为0时，才执行signal(wmutex)操作，
以便让writer进程写。又因为readcount是一个可被多个reader进程访问的临界资源，因此，也应
该为它设置一个互斥信号最nnutex

```c  title:"记录型信号屋 -> reader-writer problem"   collapse:true 

```

#### "信号量集"机制解决读者－写者问题
增加了一个限制，即最多只允许RN个读者同
时进行读操作。为此，又引入了一个信号量L, 并赋予其初值RN, 通过执行Swait(L, 1, l)操作来
控制读者的数目，每当有一个读者进行读操作时，就要先执行Swait(L,1, 1)操作以使L的值减1 。
当有RN个读者进行读操作后， L便会减为0, 此时第RN+l 个读者若要进行读操作，则必然会因
Swait(L,1,1)操作失败而阻塞。
...
Swait(mx, 1,0)语旬起着“开关”的作用。只要尤writer进程进行写操作， mx=l,
reader进程就可以读。但只要有writer进程进行写操作， mx=O, 任何reade成［程就都无法进行读
操作。Swait(mx, 1, 1 ;L,RN,0)语旬表示，仅当既无writer进程在写(mx=l) ，又无reader进程在读
(L=RN) 时， write设［程才能进入临界区写。

## 进程分析

**进程同步分析方法**  
- 找出需要同步的代码片段(关键代码)  
- 分析所找代码片段的执行次序； ＠增加同步信号最并赋初值  
